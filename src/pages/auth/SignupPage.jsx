...
import { Mail, Lock, User, Phone, Eye, EyeOff, Car, AlertCircle } from 'lucide-react'
...
export default function SignupPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await signUp(form)
      if (form.role === 'driver') navigate('/become-driver')
    ...
    <div className="min-h-screen bg-ink-50 flex items-center justify-center py-12 px-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="font-display font-bold text-2xl text-ink-900 mb-1">Create account</h1>
          <p className="text-ink-400 text-sm">Join 1.2M+ happy riders</p>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-card p-8">
          ...
          {/* Role selector */}
          <div className="grid grid-cols-2 gap-2 mb-5 p-1 bg-ink-50 rounded-2xl">
            {[{ value: 'user', label: '🚀 Rider' }, { value: 'driver', label: '🚗 Driver' }].map(({ value, label }) => (
              <button key={value} type="button"
                onClick={() => set('role', value)}
                className={`py-2.5 rounded-xl text-sm font-semibold transition-all ${form.role === value ? 'bg-white shadow-sm text-brand-600' : 'text-ink-500'}`}>
                {label}
              </button>
            ))}
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="text-xs font-semibold text-ink-700 mb-1.5 block">Phone</label>
              <div className="relative">
            ...
            <div>
              <label className="text-xs font-semibold text-ink-700 mb-1.5 block">Confirm Password</label>
              <div className="relative">
                <Lock size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-ink-400" />
          ...
          <div className="relative my-4">
            ...
            <div className="relative flex justify-center"><span className="px-3 bg-white text-ink-400 text-xs">or</span></div>
          </div>

          <button onClick={async () => { setLoading(true); try { await signInWithGoogle(); navigate('/dashboard') } catch(e){setError(e.message)} finally{setLoading(false)} }}
            disabled={loading} className="w-full btn-secondary flex items-center justify-center gap-2 text-sm">
